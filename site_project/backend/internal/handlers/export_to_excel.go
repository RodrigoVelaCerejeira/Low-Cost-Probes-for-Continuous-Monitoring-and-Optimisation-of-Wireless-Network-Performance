...
func ExportToExcel(w http.ResponseWriter, r *http.Request) {
	rows, err := database.DB.Query("SELECT * FROM dados_rede")
	if err != nil {
		log.Error(err)
	...
	w.Header().Set("Content-Disposition", "attachment;filename=dados_rede.xlsx")
	w.Header().Set("File-Name", "dados_rede.xlsx") // optional, for frontend support
	...
	log.Println("Exported to dados_rede.xlsx successfully")
}
