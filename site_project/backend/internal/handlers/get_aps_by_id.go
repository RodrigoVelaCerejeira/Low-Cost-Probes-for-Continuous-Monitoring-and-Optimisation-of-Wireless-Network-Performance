...
func GetAPsById(w http.ResponseWriter, r *http.Request) {
	id := chi.URLParam(r, "id")
	rows, err := database.DB.Query("SELECT timestamp, raspberrypi_id, ssid, bssid, rate, sig FROM all_aps WHERE raspberrypi_id = ?", id)
	if err != nil {
		log.Error(err)
